...
if(!empty($_GET['adoptno'])){

    $adoption_no=$_GET['adoptno'];
}
if (isset($_POST['add_submit'])) {
  ...
  $status = "Paid";
    ...
    amount, date) VALUES ('$payment_no', '$adoption_no', '$cardholder', '$cardnumber', '$expmonth', '$expyear',
    '$amount', '$date')")) {

        if (mysqli_query($conn, "UPDATE animal_adoption SET status = '$status' WHERE adoption_no = '$adoption_no'")){

            echo "<script>alert('Payment done successfully');location.href='adoption.php'</script>";
        } else {
